...
{
    // 定义渲染分辨率
    const uint32_t lottie_w = 460;
    const uint32_t lottie_h = 460;

    lv_obj_t *screen = lv_display_get_screen_active(disp);
    // 设置 screen 背景为白色
    lv_obj_remove_style_all(screen);                    // 清除原有样式
    ...
    // --- PSRAM 缓冲区分配 ---
    // Lottie 渲染通常需要 ARGB8888 格式（4 字节/像素）来保证透明度和抗锯齿质量
    uint32_t buf_size = lottie_w * lottie_h * 4;
    
    // 明确从 PSRAM (SPIRAM) 申请内存
    void *lottie_buffer = heap_caps_malloc(buf_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    
    if (lottie_buffer == NULL) {
        ESP_LOGE(TAG, "无法在 PSRAM 中为 Lottie 分配缓冲区！");
        return;
    }

    // 将缓冲区与 Lottie 对象关联
    // 注意：这里的 w 和 h 应与缓冲区实际对应的像素尺寸一致
    lv_lottie_set_buffer(lottie, lottie_buffer, lottie_w, lottie_h);

    // --- 加载与播放 ---
    char lottie_path[LOTTIE_PATH_MAX];
    if (!lottie_find_first_json(lottie_path, sizeof(lottie_path))) {
        ESP_LOGE(TAG, "未找到 Lottie 资源文件");
        heap_caps_free(lottie_buffer);
        return;
    }
    ...
    if (!lv_lottie_is_loaded(lottie)) {
        ESP_LOGE(TAG, "加载 Lottie 资源失败: %s", lottie_path);
        heap_caps_free(lottie_buffer);
        return;
    }
